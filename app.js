...
function section(title,value,full=false){const ls=lines(value);return'<div class="section '+(full?"full":"")+'"><h3>'+title+'</h3>'+(ls.length?'<ul>'+ls.map(x=>'<li>'+esc(x)+'</li>').join("")+'</ul>':'<p>لا توجد بيانات مضافة.</p>')+'</div>'}
function showDetail(id){const j=jobs.find(x=>x.id===id);if(!j)return;currentId=id;$("#detail").innerHTML='<div class="print-header"><div class="print-company">شركة البنية الأساسية</div><div class="print-subcompany">إدارة الموارد البشرية</div><div class="print-doc">الوصف الوظيفي</div><div class="print-meta"><span>القسم: '+esc(j.department)+'</span><span>تاريخ الإصدار: '+esc((j.updated_at||"").slice(0,10)||"—")+'</span><span>رقم الوثيقة: JD-'+esc(String(j.id||"").slice(0,8).toUpperCase())+'</span></div></div><div class="detail-top"><span class="tag">'+esc(j.department)+'</span><h2>'+esc(j.name)+'</h2><p>الرئيس المباشر: '+esc(j.manager||"—")+' • آخر تحديث: '+esc((j.updated_at||"").slice(0,10)||"—")+'</p><div class="detail-actions"><button class="btn gold" onclick="window.print()">طباعة / حفظ PDF</button><button class="btn ghost" style="color:var(--navy)" onclick="editJob(currentId)">تعديل</button><button class="btn ghost" style="color:var(--danger)" onclick="deleteJob(currentId)">حذف</button></div></div><div class="detail-grid"><div class="section full"><h3>الهدف العام من الوظيفة</h3><p>'+esc(j.purpose||"لا يوجد")+'</p></div>'+section("المهام والمسؤوليات الرئيسية",j.responsibilities,true)+section("المهام اليومية / الدورية",j.routine)+section("الصلاحيات والمسؤوليات",j.authorities)+section("المؤهلات",j.qualifications)+section("الخبرات والمهارات",j.skills)+section("مؤشرات الأداء KPI",j.kpi)+section("متطلبات الالتزام والانضباط",j.compliance,true)+'</div>';$("#detailModal").classList.remove("hidden")}
